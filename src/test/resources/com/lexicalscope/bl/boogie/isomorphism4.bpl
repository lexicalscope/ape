...
{
   (forall $a:Ref :: {$Written($h_1,$a)} {$Written($h_2,$a)} (forall <alpha> $f:Field alpha ::  $Read($h_1, $a, $f) == $Read($h_2, $a, $f)))
}
...
{ 
   ...
   assume (forall $f:Field Ref :: $Written($h_1,$Read($h_1, x, $f)));
   tE_1 := $Read($h_1, x, $E); assert($Allocated($h_1, tE_1)); assume $Written($h_1,tE_1);
   tF_1 := $Read($h_1, x, $F); assert($Allocated($h_1, tF_1)); assume $Written($h_1,tF_1);
   $h_1 := $Write($h_1, x, $E, tF_1); assume $GoodHeap($h_1); assume $Written($h_1,x);
   $h_1 := $Write($h_1, x, $F, tE_1); assume $GoodHeap($h_1); assume $Written($h_1,x);
   

   assume (forall $f:Field Ref :: $Written($h_2,$Read($h_2, x, $f)));
   tE_2 := $Read($h_2, x, $E); assert($Allocated($h_2, tE_2)); assume $Written($h_2,tE_2);
   tF_2 := $Read($h_2, x, $F); assert($Allocated($h_2, tF_2)); assume $Written($h_2,tF_2);
   $h_2 := $Write($h_2, x, $E, tF_2); assume $GoodHeap($h_2); assume $Written($h_2,x);
   $h_2 := $Write($h_2, x, $F, tE_2); assume $GoodHeap($h_2); assume $Written($h_2,x);
	...
	requires (forall a:Ref :: $Allocated($h_1, a) == $Root($roots, a));
{
	...
	assert (ta_1 == ta_2 && tb_1 == tb_2 ==> $Heap#Equal($h_1, $h_2)) || 
