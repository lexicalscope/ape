     ...
     /* modifies anything */  
{
	...
	y_0 := $Null;
			...
			if(true )
			{
				$t#0_0 := $Read($h_0,y_0,$field#v) ;
				assume $ReadObject($h_0, y_0);
				assume $ReadObject($h_0, $Read($h_0,y_0,$field#v) );
			}
			if(true )
			{
				$h_0:=$Write($h_0,x_0,$field#v,$t#0_0); assume $GoodHeap($h_0);
			}
	...
	requires (forall $a:Ref :: $Reachable($h, $roots, $a) ==> $Allocated($h, $a)); // should be an axiom of well formed heaps
{
    ...
    assume $ReadObject($h,y);
		    ...
		    if(true )
		    {
		    	$t#0_0$0 := $Read($h_0$0,y_0$0,$field#v) ;
		    	assume $ReadObject($h_0$0, y_0$0);
		    	assume $ReadObject($h_0$0, $Read($h_0$0,y_0$0,$field#v) );
		    }
		    if(true )
		    {
		    	$h_0$0:=$Write($h_0$0,x_0$0,$field#v,$t#0_0$0); assume $GoodHeap($h_0$0);
		    }
