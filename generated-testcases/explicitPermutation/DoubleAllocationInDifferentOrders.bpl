...
function $abs_DoubleAllocation_0($strategy:int, $h_pre:Heap, x_0:Ref, $h_post:Heap):bool;

// version _0 of the procedure
procedure DoubleAllocation_0($strategy:int, $h:Heap, $roots:Roots, x:Ref) returns ($h_0:Heap)
    ...
    free ensures $abs_DoubleAllocation_0($strategy, $h, x, $h_0);
...
// abstraction of function behaviour
function $abs_DoubleAllocation_1($strategy:int, $h_pre:Heap, x_1:Ref, $h_post:Heap):bool;

// version _1 of the procedure
procedure DoubleAllocation_1($strategy:int, $h:Heap, $roots:Roots, x:Ref) returns ($h_1:Heap)
    requires $Allocated($h, x);
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
    requires $Roots#Allocated($roots, $h);
    ...
    free ensures $abs_DoubleAllocation_1($strategy, $h, x, $h_1);
     ...
     /* modifies anything */  
{
	...
	x_1 := $Null;
			...
			if(true )
			{
				$h_1:=$Write($h_1,x_1,$field#f,u_1); assume $GoodHeap($h_1);
			}
...
axiom (forall 
			...
			{
				$abs_DoubleAllocation_0($allocator, $h0_0 , x_0, $hn_0) ,
				$abs_DoubleAllocation_1($allocator, $h0_1 , x_1, $hn_1) 
			}
			$abs_DoubleAllocation_0($allocator, $h0_0 , x_0, $hn_0) &&
			$abs_DoubleAllocation_1($allocator, $h0_1 , x_1, $hn_1) &&
...
// product procedure
procedure DoubleAllocation_DoubleAllocation($h:Heap, $roots:Roots, x:Ref)
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
	requires $Roots#Allocated($roots, $h);
	requires $Allocated($h, x);
	...
	requires (forall $a:Ref :: $Reachable($h, $roots, $a) ==> $Allocated($h, $a)); // should be an axiom of well formed heaps
{
    ...
    assume $ReadObject($h,x);
		    ...
		    if(true )
		    {
		    	$h_1$0:=$Write($h_1$0,x_1$0,$field#f,u_1$0); assume $GoodHeap($h_1$0);
		    }
		    ...
		    if(true )
		    {
		    	$h_1$1:=$Write($h_1$1,x_1$1,$field#f,u_1$1); assume $GoodHeap($h_1$1);
		    }
