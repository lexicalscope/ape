     ...
     /* modifies anything */  
{
	...
	y_1 := $Null;
			...
			if(true )
			{
				$h_1:=$Write($h_1,y_1,$field#v,s_1); assume $GoodHeap($h_1);
			}
	...
	requires (forall $a:Ref :: $Reachable($h, $roots, $a) ==> $Allocated($h, $a)); // should be an axiom of well formed heaps
{
    ...
    assume $ReadObject($h,y);
		    ...
		    if(true )
		    {
		    	$h_1$0:=$Write($h_1$0,y_1$0,$field#v,s_1$0); assume $GoodHeap($h_1$0);
		    }
		    ...
		    if(true )
		    {
		    	$h_1$1:=$Write($h_1$1,y_1$1,$field#v,s_1$1); assume $GoodHeap($h_1$1);
		    }
