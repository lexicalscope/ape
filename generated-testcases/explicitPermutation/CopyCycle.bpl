...
function $abs_CopyRing_0($strategy:int, $h_pre:Heap, x_0:Ref,r_0:Ref, $h_post:Heap):bool;

// version _0 of the procedure
procedure CopyRing_0($strategy:int, $h:Heap, $roots:Roots, x:Ref,r:Ref) returns ($h_0:Heap)
    ...
    free ensures $abs_CopyRing_0($strategy, $h, x, r, $h_0);
...
// abstraction of function behaviour
function $abs_CopyRing_1($strategy:int, $h_pre:Heap, x_1:Ref,r_1:Ref, $h_post:Heap):bool;

// version _1 of the procedure
procedure CopyRing_1($strategy:int, $h:Heap, $roots:Roots, x:Ref,r:Ref) returns ($h_1:Heap)
    requires $Allocated($h, x) && $Allocated($h, r);
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
    requires $Roots#Allocated($roots, $h);
    ...
    free ensures $abs_CopyRing_1($strategy, $h, x, r, $h_1);
...
axiom (forall 
			...
			{
				$abs_CopyRing_0($allocator, $h0_0 , x_0, r_0, $hn_0) ,
				$abs_CopyRing_1($allocator, $h0_1 , x_1, r_1, $hn_1) 
			}
			$abs_CopyRing_0($allocator, $h0_0 , x_0, r_0, $hn_0) &&
			$abs_CopyRing_1($allocator, $h0_1 , x_1, r_1, $hn_1) &&
...
// product procedure
procedure CopyRing_CopyRing($h:Heap, $roots:Roots, x:Ref,r:Ref)
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
	requires $Roots#Allocated($roots, $h);
	requires $Allocated($h, x) && $Allocated($h, r);
...
// abstraction of function behaviour
function $abs_CopyRingUntil_0($strategy:int, $h_pre:Heap, x_0:Ref,y_0:Ref,f_0:Ref,r_0:Ref, $h_post:Heap):bool;

// version _0 of the procedure
procedure CopyRingUntil_0($strategy:int, $h:Heap, $roots:Roots, x:Ref,y:Ref,f:Ref,r:Ref) returns ($h_0:Heap)
    requires $Allocated($h, x) && $Allocated($h, y) && $Allocated($h, f) && $Allocated($h, r);
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
    requires $Roots#Allocated($roots, $h);
    free ensures (forall $a:Ref :: $Reachable($h_0, $roots, $a) ==> $Allocated($h_0, $a)); // should be an axiom of well formed heaps
    free ensures $GoodHeap($h_0);
    free ensures $HeapSucc($h, $h_0); // this maybe introduces performance issues
    free ensures $abs_CopyRingUntil_0($strategy, $h, x, y, f, r, $h_0);
    free ensures $Heap#Bigger($h, $h_0);
    free ensures (forall $a:Ref :: // stuff is not pulled out of the garbage
					$Reachable($h_0, $roots, $a) ==>
						$Reachable($h, $roots, $a) || 
						!$Allocated($h, $a) || 
						$ReachableFromParams#4($h , x, y, f, r, $a)); 
	free ensures (forall <alpha> $a:Ref,$f:Field alpha :: // only reachable stuff is modified 
					$a != $Null && $Allocated($h,$a) && $Read($h,$a,$f)!=$Read($h_0,$a,$f) ==> 
						$ReachableFromParams#4($h , x, y, f, r, $a));
     /* modifies anything */  
...
// abstraction of function behaviour
function $abs_CopyRingUntil_1($strategy:int, $h_pre:Heap, x_1:Ref,y_1:Ref,f_1:Ref,r_1:Ref, $h_post:Heap):bool;

// version _1 of the procedure
procedure CopyRingUntil_1($strategy:int, $h:Heap, $roots:Roots, x:Ref,y:Ref,f:Ref,r:Ref) returns ($h_1:Heap)
    requires $Allocated($h, x) && $Allocated($h, y) && $Allocated($h, f) && $Allocated($h, r);
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
    requires $Roots#Allocated($roots, $h);
    ...
    free ensures $GoodHeap($h_1);
    free ensures $HeapSucc($h, $h_1); // this maybe introduces performance issues
    free ensures $abs_CopyRingUntil_1($strategy, $h, x, y, f, r, $h_1);
    free ensures $Heap#Bigger($h, $h_1);
    free ensures (forall $a:Ref :: // stuff is not pulled out of the garbage
					$Reachable($h_1, $roots, $a) ==>
						$Reachable($h, $roots, $a) || 
...
axiom (forall 
            $allocator:int,
            $h0_0:Heap, x_0:Ref,y_0:Ref,f_0:Ref,r_0:Ref, $hn_0:Heap,
			$h0_1:Heap, x_1:Ref,y_1:Ref,f_1:Ref,r_1:Ref, $hn_1:Heap ::
			{
				$abs_CopyRingUntil_0($allocator, $h0_0 , x_0, y_0, f_0, r_0, $hn_0) ,
				$abs_CopyRingUntil_1($allocator, $h0_1 , x_1, y_1, f_1, r_1, $hn_1) 
			}
			$abs_CopyRingUntil_0($allocator, $h0_0 , x_0, y_0, f_0, r_0, $hn_0) &&
			$abs_CopyRingUntil_1($allocator, $h0_1 , x_1, y_1, f_1, r_1, $hn_1) &&
			$Heap#EqualFromParams#4($h0_0 , x_0, y_0, f_0, r_0, $h0_1 , x_1, y_1, f_1, r_1) ==>
			$Heap#EqualFromParams#4($hn_0 , x_0, y_0, f_0, r_0, $hn_1 , x_1, y_1, f_1, r_1) &&
			$Heap#SameReachableFromParams#4($hn_0 , x_0, y_0, f_0, r_0, $hn_1 , x_1, y_1, f_1, r_1) &&
			$SameDiff($h0_0, $hn_0, $h0_1, $hn_1));


// product procedure
procedure CopyRingUntil_CopyRingUntil($h:Heap, $roots:Roots, x:Ref,y:Ref,f:Ref,r:Ref)
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
	requires $Roots#Allocated($roots, $h);
	requires $Allocated($h, x) && $Allocated($h, y) && $Allocated($h, f) && $Allocated($h, r);
