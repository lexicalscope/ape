...
function $abs_F_0($strategy:int, $h_pre:Heap, x_0:Ref, $h_post:Heap):bool;

// version _0 of the procedure
procedure F_0($strategy:int, $h:Heap, $roots:Roots, x:Ref) returns ($h_0:Heap)
    ...
    free ensures $abs_F_0($strategy, $h, x, $h_0);
...
// abstraction of function behaviour
function $abs_F_1($strategy:int, $h_pre:Heap, x_1:Ref, $h_post:Heap):bool;

// version _1 of the procedure
procedure F_1($strategy:int, $h:Heap, $roots:Roots, x:Ref) returns ($h_1:Heap)
    requires $Allocated($h, x);
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
    requires $Roots#Allocated($roots, $h);
    ...
    free ensures $abs_F_1($strategy, $h, x, $h_1);
...
axiom (forall 
			...
			{
				$abs_F_0($allocator, $h0_0 , x_0, $hn_0) ,
				$abs_F_1($allocator, $h0_1 , x_1, $hn_1) 
			}
			$abs_F_0($allocator, $h0_0 , x_0, $hn_0) &&
			$abs_F_1($allocator, $h0_1 , x_1, $hn_1) &&
...
// product procedure
procedure F_F($h:Heap, $roots:Roots, x:Ref)
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
	requires $Roots#Allocated($roots, $h);
	requires $Allocated($h, x);
     ...
     /* modifies anything */  
{
	...
	y_1 := $Null;
			...
			$h_1:=$Write($h_1,s_1,$field#v,t_1); assume $GoodHeap($h_1);
			$t#2_1 := $Read($h_1,s_1,$field#v) ;
			assume $ReadObject($h_1, s_1);
			assume $ReadObject($h_1, $Read($h_1,s_1,$field#v) );
			$h_1:=$Write($h_1,y_1,$field#v,$t#2_1); assume $GoodHeap($h_1);
	...
	requires (forall $a:Ref :: $Reachable($h, $roots, $a) ==> $Allocated($h, $a)); // should be an axiom of well formed heaps
{
    ...
    assume $ReadObject($h,y);
		    ...
		    $h_1$0:=$Write($h_1$0,s_1$0,$field#v,t_1$0); assume $GoodHeap($h_1$0);
		    $t#2_1$0 := $Read($h_1$0,s_1$0,$field#v) ;
		    assume $ReadObject($h_1$0, s_1$0);
		    assume $ReadObject($h_1$0, $Read($h_1$0,s_1$0,$field#v) );
		    $h_1$0:=$Write($h_1$0,y_1$0,$field#v,$t#2_1$0); assume $GoodHeap($h_1$0);
		    ...
		    assume $ReadObject($h_0$1, $t#0_0$1);
		     call $h_0$1:=F_0(1, $h_0$1, $roots, x_0$1); 
		    $t#1_0$1 := $a#1_0$1 ;
		    ...
		    assume $ReadObject($h_1$1, $t#0_1$1);
		     call $h_1$1:=F_1(1, $h_1$1, $roots, x_1$1); 
		    $t#1_1$1 := $a#1_1$1 ;
		    assume $ReadObject($h_1$1, $a#1_1$1);
		    t_1$1 := $t#1_1$1 ;
		    assume $ReadObject($h_1$1, $t#1_1$1);
		    $h_1$1:=$Write($h_1$1,s_1$1,$field#v,t_1$1); assume $GoodHeap($h_1$1);
		    $t#2_1$1 := $Read($h_1$1,s_1$1,$field#v) ;
		    assume $ReadObject($h_1$1, s_1$1);
		    assume $ReadObject($h_1$1, $Read($h_1$1,s_1$1,$field#v) );
		    $h_1$1:=$Write($h_1$1,y_1$1,$field#v,$t#2_1$1); assume $GoodHeap($h_1$1);
