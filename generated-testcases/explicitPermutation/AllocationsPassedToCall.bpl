...
function $abs_Callee_0($strategy:int, $h_pre:Heap, x_0:Ref,y_0:Ref, $h_post:Heap):bool;

// version _0 of the procedure
procedure Callee_0($strategy:int, $h:Heap, $roots:Roots, x:Ref,y:Ref) returns ($h_0:Heap)
    ...
    free ensures $abs_Callee_0($strategy, $h, x, y, $h_0);
...
// abstraction of function behaviour
function $abs_Callee_1($strategy:int, $h_pre:Heap, x_1:Ref,y_1:Ref, $h_post:Heap):bool;

// version _1 of the procedure
procedure Callee_1($strategy:int, $h:Heap, $roots:Roots, x:Ref,y:Ref) returns ($h_1:Heap)
    requires $Allocated($h, x) && $Allocated($h, y);
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
    requires $Roots#Allocated($roots, $h);
    ...
    free ensures $abs_Callee_1($strategy, $h, x, y, $h_1);
...
axiom (forall 
			...
			{
				$abs_Callee_0($allocator, $h0_0 , x_0, y_0, $hn_0) ,
				$abs_Callee_1($allocator, $h0_1 , x_1, y_1, $hn_1) 
			}
			$abs_Callee_0($allocator, $h0_0 , x_0, y_0, $hn_0) &&
			$abs_Callee_1($allocator, $h0_1 , x_1, y_1, $hn_1) &&
...
// product procedure
procedure Callee_Callee($h:Heap, $roots:Roots, x:Ref,y:Ref)
    requires $GoodHeap($h);
    requires $GoodRoots($roots);
	requires $Roots#Allocated($roots, $h);
	requires $Allocated($h, x) && $Allocated($h, y);
     ...
     /* modifies anything */  
{
	...
	x_0 := $Null;
			...
			assume $ReadObject($h_0, $t#1_0);
			 call $h_0:=Callee_0(0, $h_0, $roots, x_0, t_0); 
			$t#2_0 := $Read($h_0,t_0,$field#v) ;
			assume $ReadObject($h_0, t_0);
			assume $ReadObject($h_0, $Read($h_0,t_0,$field#v) );
			$h_0:=$Write($h_0,x_0,$field#f,$t#2_0); assume $GoodHeap($h_0);
			$h_0:=$Write($h_0,x_0,$field#g,u_0); assume $GoodHeap($h_0);
     ...
     /* modifies anything */  
{
	...
	x_1 := $Null;
			...
			assume $ReadObject($h_1, $t#1_1);
			 call $h_1:=Callee_1(0, $h_1, $roots, x_1, u_1); 
			$t#2_1 := $Read($h_1,u_1,$field#v) ;
			assume $ReadObject($h_1, u_1);
			assume $ReadObject($h_1, $Read($h_1,u_1,$field#v) );
			$h_1:=$Write($h_1,x_1,$field#f,$t#2_1); assume $GoodHeap($h_1);
			$h_1:=$Write($h_1,x_1,$field#g,t_1); assume $GoodHeap($h_1);
	...
	requires (forall $a:Ref :: $Reachable($h, $roots, $a) ==> $Allocated($h, $a)); // should be an axiom of well formed heaps
{
    ...
    assume $ReadObject($h,x);
		    ...
		    assume $ReadObject($h_0$0, $t#1_0$0);
		     call $h_0$0:=Callee_0(0, $h_0$0, $roots, x_0$0, t_0$0); 
		    $t#2_0$0 := $Read($h_0$0,t_0$0,$field#v) ;
		    assume $ReadObject($h_0$0, t_0$0);
		    assume $ReadObject($h_0$0, $Read($h_0$0,t_0$0,$field#v) );
		    $h_0$0:=$Write($h_0$0,x_0$0,$field#f,$t#2_0$0); assume $GoodHeap($h_0$0);
		    $h_0$0:=$Write($h_0$0,x_0$0,$field#g,u_0$0); assume $GoodHeap($h_0$0);
		    ...
		    assume $ReadObject($h_1$0, $t#1_1$0);
		     call $h_1$0:=Callee_1(0, $h_1$0, $roots, x_1$0, u_1$0); 
		    $t#2_1$0 := $Read($h_1$0,u_1$0,$field#v) ;
		    assume $ReadObject($h_1$0, u_1$0);
		    assume $ReadObject($h_1$0, $Read($h_1$0,u_1$0,$field#v) );
		    $h_1$0:=$Write($h_1$0,x_1$0,$field#f,$t#2_1$0); assume $GoodHeap($h_1$0);
		    $h_1$0:=$Write($h_1$0,x_1$0,$field#g,t_1$0); assume $GoodHeap($h_1$0);
		    ...
		    assume $ReadObject($h_0$1, $t#1_0$1);
		     call $h_0$1:=Callee_0(1, $h_0$1, $roots, x_0$1, t_0$1); 
		    $t#2_0$1 := $Read($h_0$1,t_0$1,$field#v) ;
		    assume $ReadObject($h_0$1, t_0$1);
		    assume $ReadObject($h_0$1, $Read($h_0$1,t_0$1,$field#v) );
		    $h_0$1:=$Write($h_0$1,x_0$1,$field#f,$t#2_0$1); assume $GoodHeap($h_0$1);
		    $h_0$1:=$Write($h_0$1,x_0$1,$field#g,u_0$1); assume $GoodHeap($h_0$1);
		    ...
		    assume $ReadObject($h_1$1, $t#1_1$1);
		     call $h_1$1:=Callee_1(1, $h_1$1, $roots, x_1$1, u_1$1); 
		    $t#2_1$1 := $Read($h_1$1,u_1$1,$field#v) ;
		    assume $ReadObject($h_1$1, u_1$1);
		    assume $ReadObject($h_1$1, $Read($h_1$1,u_1$1,$field#v) );
		    $h_1$1:=$Write($h_1$1,x_1$1,$field#f,$t#2_1$1); assume $GoodHeap($h_1$1);
		    $h_1$1:=$Write($h_1$1,x_1$1,$field#g,t_1$1); assume $GoodHeap($h_1$1);
