...
procedure G_0($strategy:int, $h:Heap, $roots:Roots, x:Ref,y:Ref) returns ($h_0:Heap)
    ...
    free ensures (forall $a:Ref :: // stuff is made reachable only if something in the modifies set is reachable
    				$ReachableFromParams#1($h_0, y, $a) && !$ReachableFromParams#1($h, y, $a) ==>
    				    $Allocated($h_0, $a) &&
    					($ReachableFromParams#1($h, y, x ))
    );
     
{
	...
	y_0 := $Null;
			...
			if(true )
			{
				$t#0_0 := $Read($h_0,x_0,$field#v) ;
				assume $ReadObject($h_0, x_0);
				assume $ReadObject($h_0, $Read($h_0,x_0,$field#v) );
			}
			if(true )
			{
				$h_0:=$Write($h_0,y_0,$field#v,$t#0_0); assume $GoodHeap($h_0);
			}
...
procedure G_1($strategy:int, $h:Heap, $roots:Roots, x:Ref,y:Ref) returns ($h_1:Heap)
    ...
    free ensures (forall $a:Ref :: // stuff is made reachable only if something in the modifies set is reachable
    				$ReachableFromParams#1($h_1, y, $a) && !$ReachableFromParams#1($h, y, $a) ==>
    				    $Allocated($h_1, $a) &&
    					($ReachableFromParams#1($h, y, x ))
    );
     
{
	...
	y_1 := $Null;
			...
			if(true )
			{
				$h_1:=$Write($h_1,y_1,$field#v,$t#0_1); assume $GoodHeap($h_1);
			}
	...
	requires (forall $a:Ref :: $Reachable($h, $roots, $a) ==> $Allocated($h, $a)); // should be an axiom of well formed heaps
{
    ...
    assume $ReadObject($h,y);
		    ...
		    if(true )
		    {
		    	$t#0_0$0 := $Read($h_0$0,x_0$0,$field#v) ;
		    	assume $ReadObject($h_0$0, x_0$0);
		    	assume $ReadObject($h_0$0, $Read($h_0$0,x_0$0,$field#v) );
		    }
		    if(true )
		    {
		    	$h_0$0:=$Write($h_0$0,y_0$0,$field#v,$t#0_0$0); assume $GoodHeap($h_0$0);
		    }
		    ...
		    if(true )
		    {
		    	$h_1$0:=$Write($h_1$0,y_1$0,$field#v,$t#0_1$0); assume $GoodHeap($h_1$0);
		    }
